...
export var d3Graphs = {
    ...
    initGraphs: function(spec) {
        this.showHud(spec);
        this.drawBarGraph(spec);
        this.drawHistogram(spec);
    },
    showHud: function(spec) {
        ...
        $("#hudButtons .countryTextInput").autocomplete({ source: spec.selectableCountries, autoFocus: true });
...
//        var windowWidth = $(window).width();
    ...
    setHistogramData:function(spec) {
        var importArray = [];
        var exportArray = [];
        var historical = spec.selectedCountry.summary.historical;
...
//            importArray.push({x:i, y:null});
    ...
    drawHistogram:function(spec) {
        ...
        var maxX = spec.selectedCountry.summary.historical.length - 1;
    ...
    drawBarGraph: function(spec) {
        ...
        var importTotal = spec.selectedCountry.summary.imported.total;
        var exportTotal = spec.selectedCountry.summary.exported.total;
        var minImExAmount = Number.MAX_VALUE;
        var maxImExAmount = Number.MIN_VALUE;
        for(var type in spec.reverseWeaponLookup) {
            var imAmnt = spec.selectedCountry.summary.imported[type];
            var exAmnt = spec.selectedCountry.summary.exported[type];
        ...
        for(var i = 0; i < importLabelArray.length; i++) {
            ...
            if(pieceHeight < smallLabelSize) {
                ...
                var textLabel = importLabel.append('text').text(function(d) {
                    return spec.reverseWeaponLookup[d.type].split(' ')[0].toUpperCase();
