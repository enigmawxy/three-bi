...
import './jquery-ui-1.8.21.custom.min';
import {dataObject} from "../js/main";
import {selectVisualization} from '../js/visualize';
import {camera} from '../js/apps';
 ...
 **/
// http://www.imooc.com/wenda/detail/587127
// 把鼠标操作带到前面
...
export var d3Graphs = {
    ...
    showHud: function() {
        ...
        $("#hudButtons .countryTextInput").autocomplete({ source: dataObject.selectableCountries, autoFocus: true });
...
//        var windowWidth = $(window).width();
    ...
    updateViz:function() {
        ...
        if(typeof dataObject.countryData[country] == 'undefined') {
            return;
        }
        ...
        for(var i = 0; i < exportBtns.length; i++) {
            ...
            var weaponName = dataObject.reverseWeaponLookup[weaponTypeKey];

            if(btn.find('.inactive').length === 0) {
                exportArray.push(weaponName);
                dataObject.selectionData.exportCategories[weaponName] = true;
            } else {
                dataObject.selectionData.exportCategories[weaponName] = false;
            }
        }
        //imports esecond
        var importArray = [];
        var importBtns = $("#importExportBtns .imports>div").not(".label");
        for(i = 0; i < importBtns.length; i++) {
            btn = $(importBtns[i]);
            weaponTypeKey = btn.attr('class');
            weaponName = dataObject.reverseWeaponLookup[weaponTypeKey];
            if(btn.find('.inactive').length === 0) {
                importArray.push(weaponName);
                dataObject.selectionData.importCategories[weaponName] = true;
            } else {
                dataObject.selectionData.importCategories[weaponName] = false;
            }
        }
        dataObject.selectionData.selectedYear = year;
        dataObject.selectionData.selectedCountry = country;
        selectVisualization(year,[country],exportArray, importArray);
    },
    dropHandle:function() {
        d3Graphs.updateViz();
    },
    ...
    setHistogramData:function() {
        var importArray = [];
        var exportArray = [];
        var historical = dataObject.selectedCountry.summary.historical;
...
//            importArray.push({x:i, y:null});
    ...
    drawHistogram:function() {
        ...
        var maxX = dataObject.selectedCountry.summary.historical.length - 1;
    ...
    drawBarGraph: function() {
        ...
        var importTotal = dataObject.selectedCountry.summary.imported.total;
        var exportTotal = dataObject.selectedCountry.summary.exported.total;
        var minImExAmount = Number.MAX_VALUE;
        var maxImExAmount = Number.MIN_VALUE;
        for(var type in dataObject.reverseWeaponLookup) {
            var imAmnt = dataObject.selectedCountry.summary.imported[type];
            var exAmnt = dataObject.selectedCountry.summary.exported[type];
        ...
        var importRects =this.barGraphSVG.selectAll("rect.import")
            .data(importArray); // 数据绑定
        importRects.enter()  // 处理数据个数与便签个数不同的情况
            .append('rect')
        ...
        importRects.attr('y',function(d) {
                var value = d3Graphs.barGraphHeight - d3Graphs.barGraphBottomPadding
        ...
        var exportRects = this.barGraphSVG.selectAll('rect.export')
            .data(exportArray);
        exportRects.enter()
            .append('rect')
            .attr('class',function(d) {
        ...
        for(var i = 0; i < importLabelArray.length; i++) {
            ...
            if(pieceHeight < smallLabelSize) {
                ...
                var textLabel = importLabel.append('text').text(function(d) {
                    return dataObject.reverseWeaponLookup[d.type].split(' ')[0].toUpperCase();
        ...
        var importLabel_b = this.barGraphSVG.selectAll('text.importLabel').data([1]);
        importLabel_b.enter().append('text').attr('x',midX).attr('text-anchor','end').text('IMPORTS')
            .attr('class','importLabel').attr('y', this.barGraphHeight - this.barGraphBottomPadding + 45);
        importLabel_b.attr('visibility',importsVisible ? "visible":"hidden");
        //Export label at bottom
        var exportLabel_b = this.barGraphSVG.selectAll('text.exportLabel').data([1]);
        exportLabel_b.enter().append('text').attr('x',midX+10).text('EXPORTS')
            .attr('class','exportLabel').attr('y', this.barGraphHeight - this.barGraphBottomPadding + 45);
        exportLabel_b.attr('visibility',exportsVisible ? "visible":"hidden")
    },
